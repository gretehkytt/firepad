...
//var rtcm = firepad.RichTextCodeMirrorAdapter;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  target.addEventListener('mousemove', function mousemove(event) {
...
firepad.FirebaseAdapter = (function (global) {
  ...
  function FirebaseAdapter (ref, userId, userColor) {
    ...
    if (userId) {
      ...
      // Once we're initialized, start tracking users' cursors and mice positions.
  ...
  utils.makeEventEmitter(FirebaseAdapter, ['ready', 'cursor', 'operation', 'ack', 'retry', 'mouse']);
  ...
  FirebaseAdapter.prototype.setUserId = function(userId) {
    if (this.userRef_) {
      ...
      this.userRef_.child('mouse').remove();
      this.userRef_.child('mouse').onDisconnect().cancel();
    }
  ...
  FirebaseAdapter.prototype.sendMouse = function (obj) {
  ...
  FirebaseAdapter.prototype.monitorMice_ = function() {
    var usersRef = this.ref_.child('users'), self = this;
...
firepad.EditorClient = (function () {
  ...
  function EditorClient (serverAdapter, editorAdapter) {
    ...
    this.editorAdapter.registerCallbacks({
      ...
      mouseActivity: function(event) {self.onMouseActivity(event); }
    });
  ...
  EditorClient.prototype.updateMouse = function(event) {
    this.mouse = this.editorAdapter.getMouse(event);
  };


  EditorClient.prototype.onMouseActivity = function (event) {
  ...
  EditorClient.prototype.sendMouse = function (mouse) {
     if (this.state instanceof Client.AwaitingWithBuffer) { return; }
...
firepad.RichTextCodeMirror = (function () {
  ...
  function RichTextCodeMirror(codeMirror, entityManager, options) {
    ...
    this.codeMirror.on('mouseActivity', this.onMouseActivity_);

    this.changeId_ = 0;
    this.outstandingChanges_ = { };
    this.dirtyLines_ = [];
  }
  utils.makeEventEmitter(RichTextCodeMirror, ['change', 'attributesChange', 'newLine', 'mouseActivity']);
  ...
  RichTextCodeMirror.prototype.detach = function() {
    ...
    this.codeMirror.off('mouseActivity', this.onMouseActivity_);
  ...
  RichTextCodeMirror.prototype.onMouseActivity_ = function() {
    console.log("moving");
  };
...
firepad.RichTextCodeMirrorAdapter = (function () {
  ...
  function RichTextCodeMirrorAdapter (rtcm) {
    ...
    this.rtcm.on('mouseActivity', this.onMouseActivity);
  ...
  RichTextCodeMirrorAdapter.prototype.detach = function () {
    ...
    this.rtcm.off('mouseActivity', this.onMouseActivity);
  ...
  RichTextCodeMirrorAdapter.prototype.onMouseActivity = function () {
    console.log("moving");
  ...
  RichTextCodeMirrorAdapter.prototype.getMouse = function (event) {
    return new Mouse(event.clientX, event.clientY);
  }
  ...
  RichTextCodeMirrorAdapter.prototype.setOtherMouse = function(mouse, color, clientId){
    // show mouse
    var mouseX = mouse.x;
    var mouseY = mouse.y;
...
firepad.Firepad = (function(global) {
  ...
  function Firepad(ref, place, options) {
    ...
    this.firebaseAdapter_.on('mouse', function() {
